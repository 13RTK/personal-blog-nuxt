<template>
  <div :class="cn('relative block w-full', props.class)">
    <div :class="cn(
      ...
      <div v-for="(item, index) in props.cards" :key="index" ref="card"
        class="card block rounded border border-transparent px-3 py-5 transition-all duration-200" :style="{
          zIndex: index + 1,
        }">
        <NuxtLink :to="item.link" target="_blank">
          <div class="tooltip tooltip-bottom" :data-tip="item.desc">
