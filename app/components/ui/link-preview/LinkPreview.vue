<template>
  <div
    :class="cn('relative inline-block text-xl text-gray-500 underline hover:text-gray-800 hover:decoration-2 focus:outline-hidden focus:decoration-2 dark:text-neutral-500 dark:hover:text-neutral-400', props.class)">
    <!-- Trigger -->
    <NuxtLink :to="url" :class="cn(props.linkClass)" @mousemove="handleMouseMove" @mouseenter="showPreview"
      @mouseleave="hidePreview" target="_blank">
      <!-- <div v-if="isLoading && isVisible"
        class="absolute inset-0 z-50 flex items-center justify-center bg-white/50 dark:bg-black/50">
